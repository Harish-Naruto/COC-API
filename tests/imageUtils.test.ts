...
describe("imageUtils", () => {
  ...
  describe("uploadImage", () => {
    it("uploads an image successfully and returns public URL", async () => {
      ...
      storageMock.getPublicUrl.mockReturnValue({
        data: { publicUrl: "https://public.url/file.png" },
      });
    ...
    it("throws ApiError when upload fails", async () => {
      storageMock.upload.mockResolvedValue({
        error: { message: "fail" } as PostgrestError,
      });
      await expect(
  ...
  describe("deleteImage", () => {
    it("deletes an image successfully", async () => {
      const publicUrl =
        "https://xyz.supabase.co/storage/v1/object/public/images/folder/file.png";
    ...
    it("throws ApiError when delete fails", async () => {
      const publicUrl =
        "https://xyz.supabase.co/storage/v1/object/public/images/folder/file.png";
      storageMock.remove.mockResolvedValue({
        error: { message: "delete fail" } as PostgrestError,
      });
